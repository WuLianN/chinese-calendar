...
import { JD, XL, J2000, pi2, int2, dt_T  } from './eph0.js'
...
class oba { //公历基础构件
 //private私有成员定义
 static wFtv = new Array( //某月的第几个星期几,如第2个星期一指从月首开始顺序找到第2个“星期一”
 ...
 static sFtv = '' //假日表,由init初始化

 //public公有成员定义
 static init(){ //初始化
  let s =  //国历节日,#表示放假日,I表示重要节日或纪念日
   '01#元旦|' //1月
  ...
  this.sFtv=s.split('|');
  for(let i=0;i<this.sFtv.length;i++) this.sFtv[i]=this.sFtv[i].split(',');
 }

 static getDayName(u,r){ //取某日节日,传入日对象
  /****************
   节日名称生成
   传入日物件u 
   返回某日节日信息
  ...
  *****************/
  ...
  //按公历日期查找
  for(let i=0;i<this.sFtv[u.m-1].length;i++){ //公历节日或纪念日,遍历本月节日表
   s=this.sFtv[u.m-1][i];
  ...
  //按周查找
  let w=u.weeki; if(u.week>=u.week0) w+=1;
  let w2=w;      if(u.weeki==u.weekN-1) w2=5;
  w = m0 + w  + u.week;  //d日在本月的第几个星期某
  w2= m0 + w2 + u.week;

  for(let i=0;i<this.wFtv.length;i++){
   s=this.wFtv[i];
   s2=s.substr(0,4);
...
class obb { //农历基础构件
 //private私有成员定义
 static JNB  = '' //纪年表,由init初始化

 //public公有成员定义
 //以下几个是公有只读数据
 static numCn = new Array('零','一','二','三','四','五','六','七','八','九','十') //中文数字
 static Gan = new Array("甲","乙","丙","丁","戊","己","庚","辛","壬","癸")
 static Zhi = new Array("子","丑","寅","卯","辰","巳","午","未","申","酉","戌","亥")
 static ShX = new Array("鼠","牛","虎","兔","龙","蛇","马","羊","猴","鸡","狗","猪")
 static XiZ = new Array('摩羯','水瓶','双鱼','白羊','金牛','双子','巨蟹','狮子','处女','天秤','天蝎','射手')
 static yxmc = new Array("朔","上弦","望","下弦") //月相名称表
 static jqmc = new Array('冬至','小寒','大寒','立春','雨水','惊蛰','春分','清明','谷雨','立夏','小满','芒种','夏至','小暑','大暑','立秋','处暑','白露','秋分','寒露','霜降','立冬','小雪','大雪')
 static ymc = new Array('冬','腊','正','二','三','四','五','六','七','八','九','十') //月名称,建寅
 static rmc = new Array('初一','初二','初三','初四','初五','初六','初七','初八','初九','初十','十一','十二','十三','十四','十五','十六','十七','十八','十九','二十','廿一','廿二','廿三','廿四','廿五','廿六','廿七','廿八','廿九','三十','卅一')
 ...
 static init(){ //初始化
  ...
  this.JNB = s.split(',');
  for(let i=0;i<this.JNB.length;i+=7) { this.JNB[i]-=0; this.JNB[i+1]-=0; this.JNB[i+2]-=0; }
 }

 static getNH(y){ //取年号
  let j,c,s='', ob=this.JNB;
 ...
 static getDayName(u,r){ //计算农历节日
  //按农历日期查找重量点节假日
  let d=u.Lmc + (u.Lmc.length<2?'月':'') + u.Ldc;
  if(u.Lleap!='闰'){
 ...
 static mingLiBaZi(jd,J,ob){ //命理八字计算。jd为格林尼治UT(J2000起算),J为本地经度,返回在物件ob中
  let c, v;
  let jd2 = jd+dt_T(jd); //力学时
  let w = XL.S_aLon( jd2/36525, -1 ); //此刻太阳视黄经
  ...
  let D = Math.floor(jd), SC = int2( (jd-D)*12 ); //日数与时辰
  ...
  v-= SC, ob.bz_JS = ''; //全天纪时表
  for(let i=0; i<13; i++){ //一天中包含有13个纪时
 ...
 static qi_accurate(W)  { let t=XL.S_aLon_t(W)*36525;  return t - dt_T(t) + 8/24; } //精气
 static so_accurate(W)  { let t=XL.MS_aLon_t(W)*36525; return t - dt_T(t) + 8/24; } //精朔
 static qi_accurate2(jd) { //精气
  let d = Math.PI/12;
  let w=Math.floor((jd+293)/365.2422*24) * d;
  let a= this.qi_accurate ( w );
  if(a-jd>5 ) return this.qi_accurate ( w-d );
  if(a-jd<-5) return this.qi_accurate ( w+d );
  return a;
 }
 static so_accurate2(jd) { return this.so_accurate ( Math.floor((jd+8)/29.5306) * Math.PI*2 ); } //精朔
...
class SSQ { //实朔实气计算器
 ...
 static SB = '' //朔修正表
 static QB = '' //气修正表
 static suoKB = new Array( //朔直线拟合参数
...
//1807665.420323,29.53059851, //  237-02-12 h=0.00033 晋书·律历志(景初历)
 ...
 static so_low(W){ //低精度定朔计算,在2000年至600，误差在2小时以内(仍比古代日历精准很多)
 ...
 static qi_low(W){ //最大误差小于30分钟，平均5分
 ...
 //public公有成员定义
 static calc(jd,qs){ //jd应靠近所要取得的气朔日,qs='气'时，算节气的儒略日
  jd += 2451545;
  let D,n;
  let B=this.suoKB,pc=14; if(qs=='气') B=this.qiKB,pc=7;
  ...
  if( jd>=f1 && jd<f2 ) { //平气或平朔
   for(let i=0;i<B.length;i+=2)  if(jd+pc<B[i+2]) break;
 ...
 static leap = 0         //闰月位置
 static ym = new Array() //各月名称
 static ZQ = new Array() //中气表,其中.liqiu是节气立秋的儒略日,计算三伏时用到
 static HS = new Array() //合朔表
 static dx = new Array() //各月大小
 static Yn = new Array() //年计数

 static calcY(jd){ //农历排月序计算,可定出农历,有效范围：两个冬至之间(冬至一 <= d < 冬至二)
  let A=this.ZQ, B=this.HS;  //中气表,日月合朔表(整日)
  let W, w;
  ...
  //该年所有朔,包含14个月的始末
  for(let i=0;i<15;i++) B[i]=this.calc(w+29.5306*i,'朔');
  ...
  let YY = int2( (this.ZQ[0]+10 +180)/365.2422) + 2000; //确定年份
  if( YY>=-721 && YY <=-104 ){
   let ns = new Array(), yy;
   for(let i=0;i<3;i++){
    yy = YY+i-1;
   ...
   let f1;
   for(let i=0;i<14;i++){
    for(let nn=2;nn>=0;nn--) if(this.HS[i]>=ns[nn]) break;
  ...
  //名称转换(月建别名)
  for(let i=0;i<14;i++){
    let Dm = this.HS[i]+J2000, v2=this.ym[i]; //Dm初一的儒略日,v2为月建序号
    let mc = obb.ymc[v2%12]; //月建对应的默认月名称：建子十一,建丑十二,建寅为正……
...
export class Lunar {
 ...
 yueLiCalc(By,Bm){
  ...
  for(let i=0; i<Bdn; i++) this.lun[i] = {};
  ...
  for(let i=0;i<Bdn;i++){
    ...
    if(!SSQ.ZQ.length || ob.d0<SSQ.ZQ[0] || ob.d0>=SSQ.ZQ[24]) //如果d0已在计算农历范围内则不再计算
      SSQ.calcY(ob.d0);
    let mk = int2( (ob.d0-SSQ.HS[0])/30 );  if(mk<13 && SSQ.HS[mk+1]<=ob.d0) mk++; //农历所在月的序数
    ...
    let qk=int2( (ob.d0-SSQ.ZQ[0]-7)/15.2184 ); if(qk<23 && ob.d0>=SSQ.ZQ[qk+1]) qk++; //节气的取值范围是0-23
 ...
 getJieQiByYear(year) {
  ...
  for (let j=1; j<25; j++) {
   let T = XL.S_aLon_t( (y0+(j-6)*15/360+1)*2*Math.PI ); //精确节气时间计算
 
   let jjTime = JD.JD2str( T*36525+J2000+8/24-dt_T(T*36525) );//日期转为字串

   const struct = {
    jieQi: solarTerm[(j-1)%24],
    time: jjTime
   }
